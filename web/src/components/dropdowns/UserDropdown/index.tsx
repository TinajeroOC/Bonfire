    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Avatar className="h-10 w-10 rounded-sm">
          <AvatarImage src={session.userAttributes.avatarUrl ?? undefined} alt={session.userAttributes.username} />
          <AvatarFallback className="select-none">{session.userAttributes.username[0].toUpperCase()}</AvatarFallback>
        </Avatar>
      </DropdownMenuTrigger>
        ...
        <DropdownMenuLabel>
          <div className="flex items-center gap-3 py-1.5 text-left text-sm">
            ...
            <div className="grid flex-1 text-left text-sm leading-tight">
              ...
              <span className="truncate text-xs text-muted-foreground">{session.userAttributes.email}</span>
            </div>
          </div>
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
